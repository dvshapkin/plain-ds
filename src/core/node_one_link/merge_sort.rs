...
/// Merge sort implementation for linked single_linked_list nodes
pub fn merge_sort<T>(head: *mut Node<T>) -> *mut Node<T>
where
    T: PartialOrd + Default
{
    // Base case: empty single_linked_list or single node
    if head.is_null() || unsafe { (*head).next.is_null() } {
        return head;
    }

    // Split the single_linked_list into two halves
...
/// Splits the single_linked_list into two approximately equal halves
...
/// Merges two sorted linked lists into one sorted single_linked_list
    ...
    while !left.is_null() && !right.is_null() {
        unsafe {
            if (*left).payload <= (*right).payload {
                // Take from left single_linked_list
                (*actual_tail).next = left;
                actual_tail = left;
                left = (*left).next;
            } else {
                // Take from right single_linked_list
